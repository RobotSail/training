...
def unmask_from_ranges(input_ids: t.List[int], unmask_ranges: t.List[t.Tuple[int, int]]) -> t.List[int]:
    ...
    if not equal_lengths:
        print(f'fuck! we found a case where {len(labels)=} != {len(input_ids)=}')
        import IPython
        IPython.embed()
...
def new_main(args: NewDataProcessArgs) -> None:
    ...
    final_dataset = prepare_final_dataset(data_with_input_ids_and_labels, tokenizer, args.num_cpu_procs)
...
def prepare_final_dataset(data: Dataset, tokenizer: PreTrainedTokenizer, num_proc: int) -> Dataset:
