...
@app.command()
def granite_p07_adam_baseline(
    experiment_prefix: t.Annotated[
        str, typer.Option(help="The prefix of the experiment.")
    ] = "default-muon-exp",
    validate_every_n_steps: t.Annotated[
        int, typer.Option(help="How often to validate the model.")
    ] = 25,
):
    ...
    experiment_name = experiment_prefix + "-{time}_" + "adam-baseline"

    # first train the model with p07
    p07_data_path = "/mnt/nvme3n1/rhelai-1.5-bmo-stash/BMO/bmo_knowledge.jsonl"
    # p07_ckpt_output_dir = "/mnt/7TB-a/models/granite-3.1-8b-v2-BMO-p07_muon-1e4"
    p07_ckpt_output_dir = (
        "/mnt/nvme3n1/models/granite-3.1-8b-instruct_" + experiment_name
    )
    p07_input_model = "/mnt/nvme3n1/models/granite-3.1-8b-starter-v2"
    ...
    p07_training_args = TrainingArgs(
        ...
        run_name=experiment_name,  # Will include timestamp
        optimizer=Optimizer.ADAMW,
        eval_every_n_steps=validate_every_n_steps,
...
def granite_p07(
    ...
    LRs = [
        2e-5,  # test
        1e-5,  # we already sweeped this one so there's no point in re-running it
        3e-5,
        8e-5,
        6e-6,
        1e-4,
    ...
    for lr in LRs:
        experiment_name = experiment_prefix + "-{time}_" + str(lr)
